...
export const UNADDABLE_FIELD_TYPES: Record<string, string> = {
  ...
  // テーブルも fields に書くものではない。列になるフィールドに table を付けると、
  // fieldsmith が SUBTABLE に組み立てる。入れ子を書かせないのは group と同じ理由。
  SUBTABLE: "テーブルは fields に書かず、列になるフィールドに table を付けてください",
  // 関連レコード一覧も相手アプリに依存する。加えて condition と displayFields が
  // 相手アプリのフィールドコードなので、--dry-run では実在を確かめようがない。
  // 値を持たない表示専用のフィールドなので、外してもレコードのデータは失われない。
  // @see README.md「関連レコード一覧を対象外にしている理由」
  ...
  // ルックアップと関連レコード一覧は、外している理由が他と違う。
  // テーブルやグループがアプリ内で完結するのに対し、この 2 つは他のアプリに依存する。
  //
  //   1. 設定に相手アプリの指定が要る。spec に書くと特定の kintone 環境に縛られ、
  //      「同じ spec を別の環境にもう 1 つ」ができなくなる。
  //      space を examples/ の spec に持たせていないのと同じ理由。
  //      (アプリコードで書けば ID 依存は避けられるが、相手アプリがどの環境でも
  //       同じアプリコードで在ることが前提になり、fieldsmith には保証できない)
...
const baseFieldShape = {
  ...
  /**
   * テーブルの名前 (「明細」「担当者履歴」など)。
   * 同じ名前を書いたフィールドが、1 つのテーブルの列になる。
   *
   * `group` と同時には指定できない。kintone はテーブルの中にグループを置けないため、
   * 両方に属することがそもそも有り得ない。
   *
   * この名前がテーブルのフィールドコードの元になるので、上限はフィールドコードに合わせる。
   */
