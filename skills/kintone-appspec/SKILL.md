...
`pull` は `"layout": "stacked"`（＝既存の並びに触らない）を付けて返す。**並びを変えたいと頼まれた時だけ** `"grouped"` に書き換える。書き換えると差分に「フォームの並びを組み直す」が出る。
...
`group` は「フォーム上で横に並べる単位」で、kintone には送られない fieldsmith 内部の情報。
