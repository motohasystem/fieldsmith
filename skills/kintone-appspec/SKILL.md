...
```bash
npx fieldsmith check <spec.json> <data.csv>
```

kintone のレコード追加は 100 件のかたまり単位で失敗するので、1 件の違反が 100 件の取りこぼしになる。終了コード 2 なら、**CSV を直すのか spec の制約を直すのかをユーザーに確認する**（`maxValue 31` の「締日」に 90 が並ぶなら、そもそも日付ではなくコード値かもしれない）。

`⚠` は投入自体は通る。握りつぶさずそのまま伝える。
...
テーブルは `pull` で往復する（列が `table` 付きのフィールドとして戻る）。**ただし `update` では変えられない**ので、テーブルに関わる依頼が来たら kintone の画面で直してもらう。
...
- **既存フィールドの `table` を変えない。** テーブルの出入りは kintone では作り直しで、データが引き継がれない
- 新しいフィールドだけ `code` を空にする（`label` から導出される）
- 頼まれていないフィールド・一覧は、`pull` した内容のまま書き写す
- 変えたい設定は**明示的に書く**（書かれていない項目は現状維持）
- **制約を外すときはキーを消さない。** 空文字を書く（`"maxValue": ""`）。真偽値は `false`。
  消すだけでは kintone 側に残る。`diff` が警告で知らせるので、出たら従う
...
同じ `table` を書いたフィールドが、kintone のテーブルの列になる。**入れ子では書かない。**
...
- **同じ `table` の列は必ず続けて並べる。** `group` と同じ
- `table` と `group` は**同時に指定できない**
- テーブルの列は **`views` に書けない**
- **`update` はテーブルを変えられない。** 列の追加・削除・出し入れは止められる。
  変えたいときは kintone の画面で直して `pull` し直す、とユーザーに伝える

「明細」「内訳」「履歴」のように、**1 レコードに複数行ぶら下がるもの**にだけ使う。
繰り返しの無い項目をテーブルにしない。
...
| `LOOKUP` / `REFERENCE_TABLE`: v1 では未対応 | 別のフィールドで表現し直す。「今は作れないので手で足す必要がある」と伝える |
...
| `SUBTABLE: テーブルは fields に書かず、列になるフィールドに table を付けてください` | `fields` からその行を消し、列になるフィールドに同じ `table` を付ける |
| `table "◯◯" のフィールドが離れて書かれています` | 同じテーブルの列を続けて並べる |
| `table "◯◯" と group "△△" は同時に指定できません` | kintone はテーブルにグループを置けない。どちらかにする |
| `"◯◯" はテーブル「△△」の列なので、一覧には指定できません` | `views` の `fields` からその列を外す |
| `テーブルの変更は update では反映できません`（update） | kintone の画面でテーブルを直し、`pull` で spec を取り直す。**勝手に諦めず、この手順をユーザーに伝える** |
