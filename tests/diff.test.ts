 ...
 * 数値の設定の書き方の揺れ。
 *
 * kintone は上限・下限・初期値を**文字列で返す**が、spec には数値でも書ける。
 * 区別すると、中身が同じなのに毎回の差分に出続ける。
 */
describe("数値の設定は書き方で差分にしない", () => {
  const number = (over: Record<string, unknown>) =>
    parseAppSpec({
      name: "案件",
      layout: "stacked",
      fields: [{ type: "NUMBER", label: "締日", code: "締日", ...over }],
    });
  const changes = (current: Record<string, unknown>, desired: Record<string, unknown>) =>
    diffAppSpec(number(current), number(desired)).updated.flatMap((entry) => entry.changes);

  it("pull した \"0\" と手書きの 0 は同じ", () => {
    expect(changes({ minValue: "0", maxValue: "31" }, { minValue: 0, maxValue: 31 })).toEqual([]);
  });

  it("空文字でクリアしたことは差分に出す", () => {
    // kintone は未設定を空文字で表す。外したいという意思表示は拾う。
    expect(changes({ maxValue: "31" }, { maxValue: "" })).toEqual([
      { key: "maxValue", from: "31", to: "" },
    ]);
  });

  it("クリアしたあと、同じ spec を流しても差分に出さない", () => {
    // pull は空文字を落とすので、current 側からは消えている。
    // ここを区別すると、クリア後ずっと差分が出続ける。
    expect(changes({}, { maxValue: "" })).toEqual([]);
  });

  it("キーを消しただけでは何も起きない (現状維持)", () => {
    // 「書かれていない項目は現状維持」。クリアしたいなら空文字を書く。
