...
`LOOKUP` と `REFERENCE_TABLE` を「v1 では未対応」としているのは、
**この 2 つが他のアプリに依存する**ため。

グループとテーブルも当初は同じ並びだったが、どちらもアプリ内で完結する構造物なので、
[`layout: "sections"`](#グループフィールド-sections) と [`table`](#テーブル) で対応した。
ルックアップと関連レコード一覧は、そこが違う。

**1. AppSpec を環境非依存に保つため（主たる理由）**

ルックアップの設定には**相手アプリの指定**が要る。これを spec に書くと、その AppSpec は
特定の kintone 環境に縛られる。`space` を `examples/` の spec に持たせていないのと
同じ理由で、「同じ spec を別の環境・別のスペースにもう 1 つ」ができなくなるのは困る。

> 逃げ道が無いわけではない。`relatedApp` はアプリ ID (`app`) のほかに
> **アプリコード** (`code`) でも書けて、両方指定すると `code` が勝つ。
> ただしそれは「相手アプリがどの環境でも同じアプリコードで存在している」ことが前提で、
> fieldsmith はその前提を**作ることも確かめることもできない**。
...
`REFERENCE_TABLE` には、ルックアップと重なる理由と、この型だけの事情がある。

**1. 相手アプリに依存する（ルックアップと共通）**

`referenceTable` にも参照先アプリの指定が要る。上のアプリコードの話もそのまま当てはまる。

**2. `--dry-run` が意味を成さない**

`condition.relatedField` と `displayFields` は、**相手アプリの**フィールドコード。
どちらも `referenceTable` を書くなら必須。`--dry-run` は kintone に接続しないので、
これらが実在するかを確かめられない。

fieldsmith の検証は「**kintone に 1 リクエストも投げる前に**弾く」ことを拠り所にしている。
確かめようのない項目を通すと、その拠り所が崩れる。

**3. データを持たないので、外すコストが低い（この型だけの事情）**

関連レコード一覧は**表示だけの装置**で、レコードに値を持たない。
...
だから AppSpec から外しても、**レコードのデータは 1 件も失われない**。デプロイした後に
kintone の画面で足しても、既にあるレコードには何の影響もない。

値を持つルックアップとは、ここが違う。ルックアップの実体は `SINGLE_LINE_TEXT` /
`NUMBER` / `LINK` に設定を付けたもので、フィールドそのものにはデータが入っている。

### `pull` はどちらも黙って捨てない

| | `pull` の扱い |
|---|---|
| 関連レコード一覧 | フィールドごと落として警告に出す（値を持たないので失われるデータは無い）|
| ルックアップ | **フィールドは残し**、ルックアップ設定を落としたことを警告に出す |

ルックアップを残すのは、そこに**データが入っているから**。落とすと `update` で
削除候補に送られてしまう。残った形は `examples/蔵書.json` と同じ「`本棚ID` を
ただの文字列フィールドとして持つ」状態になる。

その spec を `update` で送り返しても、kintone 側のルックアップ設定は消えない。
フィールド設定変更 API は「[省略したパラメーターの設定は変更されません](https://cybozu.dev/ja/kintone/docs/rest-api/apps/form/update-form-fields/)」ため。

## テーブル

同じ `table` を書いたフィールドが、kintone のテーブルの列になる。
...
```jsonc
{
  "fields": [
    ...
    { "type": "NUMBER",           "label": "単価", "table": "明細", "unit": "円" },
...
**入れ子では書かない。** `group` がセクションを決めるのと同じく、フィールドに付けた印が
テーブルを決める。こうしている理由は 2 つある。

- AppSpec が平らなままなら、差分・検証・一覧の扱いが今までと変わらない
- `plan` の構造化出力はプロパティ数の上限が厳しく、入れ子を足す余裕が無い

### 決まり

- **同じ `table` は続けて並べる。** 離れていると 2 つのテーブルに割れるので検証で弾く
- `table` と `group` は**同時に指定できない**。kintone はテーブルの中にグループを置けない
- `table` の名前がテーブルのフィールドコードになる。フィールドコードと重ならない名前にする
- テーブルの列は**一覧 (`views`) に指定できない**。1 レコードに複数の値を持つため

### `update` はテーブルを変更しない

`deploy` はテーブルを作れるが、**`update` は既存のテーブルに触れない。**
列を足す・外す・出し入れしようとすると、1 リクエストも投げずに終了コード 2 で止まる。

```
  ! 数量: テーブル「明細」の列を追加 (update では反映できない)
```

kintone のテーブルには**列の退避先が無い**。グループは `layout` に行を入れ子で持てるが、
テーブルは `fields` の平らな配列で、行も入れ子も持てない。

```
GROUP    → "layout": [ ROW, ROW, ... ]
SUBTABLE → "fields": [ field, field ]
```

列を外へ出すのは親を変えることで、kintone では作り直しになり**データが失われる**。
fieldsmith の「消さずに削除候補へ移す」が成り立たないので、中途半端に適用せず止める。

テーブルを変えたいときは kintone の画面で変更し、そのあと `pull` で spec を取り直す。
...
```jsonc
{
  ...
  "layout": "sections",                         // group をグループフィールドにする
...
`grouped` の `group` は横並びのヒントで、kintone 上には現れない。
`sections` にすると、同じ `group` のフィールドが **kintone のグループフィールド**に入る。
...
```jsonc
{
  ...
  "fields": [
    ...
    { "type": "DATE",             "label": "貸出日", "group": "貸出" }
  ]
}
```

書き方は `grouped` と同じ。`group` の付いていないフィールドは、これまで通り素の行になる。

- **`group` の名前がグループのフィールドコードになる。** ラベルからコードを導くのと
  同じ規則を使うので、同じ名前からは毎回同じコードが出る。更新を繰り返しても
  グループは増えない
- そのためフィールドコードと重なる名前は使えない（検証で弾かれる）
- グループは開いた状態で作られる（畳んだ状態にするのは `_削除候補` だけ）
- **AppSpec から消えたグループは、空のまま残る。** フィールドを削除しないのと同じ理由で、
  グループも消さない。不要なら kintone の画面で削除する（`update` が警告で知らせる）
...
3. フォームの並びを整える（`layout` が `grouped` / `sections` の場合のみ。既定で有効）
...
kintone はアプリアイコンを**画像**として持つので、画像そのものからは
「🏢 を描いた」ことも「背景色に何を使った」ことも読み取れない。

そこで**アップロードするファイル名に書いておく**。

```
fieldsmith-icon-🏢-2563eb.png
```

`getAppSettings` はこのファイル名を返すので、`pull` が `icon` と `iconBackground` に
戻せる。だから `pull` → 絵文字を書き換える → `update` でアイコンを変えられる。

人が kintone の画面で差し替えた画像はこの形にならない。その場合は従来どおり
「AppSpec には含められません」と警告する。

> アイコンは**変わったときだけ**上げ直す。毎回上げると、更新のたびに
> 画像ファイルが増えていく。

### レイアウトについて

`pull` は**読み取れたものだけを名乗る**。

| フォームの状態 | `pull` が返す `layout` |
|---|---|
| グループフィールドがある | `"sections"`（`group` に所属も復元する）|
| それ以外 | `"stacked"`（既存の並びに手を触れない）|

横並びは読み取れない。1 行に 3 つ並んでいても、fieldsmith が並べたのか人が画面で
並べたのか区別が付かないので、`stacked` と言う。横に並べたい場合は
`"layout": "grouped"` に書き換える。

グループは読み取れる。レイアウトに `GROUP` があれば、それは疑いようもなくセクションなので、
`sections` と名乗って往復できるようにしてある。**`pull` した spec をそのまま `update` しても
レイアウトは書き換わらない**（両方が同じ指定・同じ並びなので差分が出ない）。
...
ただし**並べ替えそのものは差分として拾う**（`layout` が `grouped` / `sections` のとき）。

```
  ~ フォームの並びを組み直す (並び順)
```

`layout` が `stacked` の場合は「既存の並びに手を触れない」指定なので、
並べ替えても何も起きない。差分にも出さない。並びを変えたいときは `"layout": "grouped"` にする。

`sections` では、フィールドが増減したときも組み直す。レイアウトを書かないと、
足したフィールドが kintone によってフォーム末尾に置かれ、**所属するはずのグループの外**に
出てしまうため。

```
  ~ フォームの並びを組み直す (フィールドの増減)
```

### AppSpec に現れないものは、レイアウト上で保たれる

フォームには AppSpec が扱わないものが載っている。

| | AppSpec | レイアウト |
|---|---|---|
| 関連レコード一覧・ルックアップ設定 | 表現しない | 載っている |
| ラベル・スペース・罫線 | 表現しない | 載っている（`code` ではなく `elementId` を持つ）|

`update` が並びを組み直すのは AppSpec に書かれたフィールドだけで、**それ以外は
直前のフィールドの位置に戻す**。組み直しは AppSpec を起点にするので、
何もしないと消えてしまうため。

フィールドを落とすとレイアウト変更 API が「指定が足りない」で失敗し、
飾りは API が求めないぶん黙って消える。どちらも起こさない。

> 手掛かりにしていたフィールドごと削除候補へ移った場合だけ、位置を保てずに末尾へ回る。
> それでも捨てはしない。
...
ここがいちばんハマる。**キーを消す＝制約を外す**と読みたくなるが、そうはならない。

```jsonc
// 現状: maxValue 31 が設定されている
{ "type": "NUMBER", "code": "締日", "minValue": 0 }                    // → 31 のまま
{ "type": "NUMBER", "code": "締日", "minValue": 0, "maxValue": "" }    // → 外れる
```

**外すときは空文字を書く。** kintone は未設定の上限・下限・初期値を空文字で表すので、
空文字がそのまま「制限なし」を意味する。真偽値なら `false` を書く。

黙って据え置くと「差分なし＝変わっていない」と読めてしまうので、
`diff` と `update` が**外し方つきで知らせる**。

```
注意:
  ⚠ 締日: maxValue を spec から外しましたが、kintone 側の "31" は据え置かれます
    (外すには "maxValue": "" と書きます)。
```

外し方を示せるものだけが対象。`theme` のように「空にする」書き方が無いものは黙って見送る。

#### 数値は文字列でも数値でも書ける

kintone は上限・下限・初期値を**文字列で返す**（`"31"`）。spec にはどちらでも書けて、
`0` と `"0"` は同じものとして扱う。書き方の違いが差分に出ることはない。
